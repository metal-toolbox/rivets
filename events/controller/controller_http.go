...
type NatsHttpController struct {
	...
	coClientConfig  *OrchestratorApiConfig
	coClient        *co.Client
}
...
func NewNatsHTTPController(
	...
	options ...OptionNatsHttp) *NatsHttpController {
...
func newConditionsAPIClient(endpoint, token string, disableAuth bool) (*co.Client, error) {
	if disableAuth {
		return co.NewClient(
			endpoint,
		)
...
func WithOrchestratorAPIAuthDisabled() OptionNatsHttp {
	return func(n *NatsHttpController) {
		n.coClientConfig.AuthDisabled = true
	}
}
...
func (n *NatsHttpController) Run(ctx context.Context, handler TaskHandler) error {
	...
	var err error
	n.coClient, err = newConditionsAPIClient(n.coClientConfig.Endpoint, n.coClientConfig.AuthToken, true)
	if err != nil {
		return errors.Wrap(ErrHandlerInit, "error in Conditions API client init: "+err.Error())
	}
...
func (n *NatsHttpController) runTaskWithMonitor(
	...
	n.liveness = NewHttpNatsLiveness(n.coClient, task.ID, n.serverID, controllerID, statusInterval, n.logger)
	go n.liveness.StartLivenessCheckin(ctx)

	// init publisher
	publisher := NewHttpPublisher(n.serverID, task.ID, n.conditionKind, controllerID, n.coClient, n.logger)
...
func (n *NatsHttpController) fetchCondition(ctx context.Context, serverID uuid.UUID) (*condition.Condition, error) {
	...
	resp, err := n.coClient.ConditionQueuePop(ctx, n.conditionKind, serverID)
	if err != nil {
...
func (n *NatsHttpController) conditionFromResponse(resp *types.ServerResponse) (*condition.Condition, error) {
	...
	return resp.Records.Conditions[0], nil
}
...
func (n *NatsHttpController) fetchTask(ctx context.Context, serverID uuid.UUID) (*condition.Task[any, any], error) {
	...
	resp, err := n.coClient.ConditionTaskQuery(ctx, n.conditionKind, serverID)
	if err != nil {
